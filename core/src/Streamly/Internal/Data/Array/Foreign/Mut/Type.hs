...
fptrToArrayContents :: ForeignPtrContents -> ArrayContents a
arrayToFptrContents :: ArrayContents a -> ForeignPtrContents
#ifdef USE_FOREIGN_PTR
...
fromForeignPtrUnsafe fp@(ForeignPtr fpAddr contents) endPtr boundPtr =
    -- XXX PIN THE ARRAY
    -- XXX The array will likely be pinned!
    let acont@(!(ArrayContents mbarr# :: ArrayContents a)) =
            fptrToArrayContents contents
        startPtr = Ptr (byteArrayContents# (unsafeCoerce# mbarr#))
        fpPtr = Ptr fpAddr
        sizeA = SIZE_OF(a)
     in assert
            (unsafeForeignPtrToPtr fp <= endPtr && endPtr <= boundPtr)
            (Array
                 acont
                 ((startPtr `minusPtr` fpPtr) `div` sizeA)
                 ((endPtr `minusPtr` fpPtr) `div` sizeA)
                 ((boundPtr `minusPtr` fpPtr) `div` sizeA))
...
-- XXX THIS IS ONLY POSSIBLE C MALLOC
newArrayAlignedUnmanaged = undefined
#ifdef USE_FOREIGN_PTR
...
putIndex i x arr =
    putIndexPtr (arrContents arr) (arrStart arr) (aEnd arr) i x
...
putIndices Array{..} = FL.rmapM extract $ FL.foldlM' step initial
    ...
    -- We can use putIndex here
    step () (i, x) = liftIO (putIndexPtr arrContents arrStart aEnd i x)
...
modifyIndexUnsafe i f (Array{..}) = liftIO $ do
        let index = INDEX_OF(arrStart,i)
        assert (i >= 0 && INDEX_NEXT(index) <= aEnd) (return ())
...
modifyIndexPtr :: forall m a b. (MonadIO m, Storable a) =>
    ArrayContents a -> Int -> (a -> (a, b)) -> Int -> Int -> m b
modifyIndexPtr arrContents i f start end = liftIO $ do
...
swapIndices i1 i2 Array{..} = liftIO $ do
        ...
        when (i1 < 0 || INDEX_INVALID(t1,aEnd))
...
snocUnsafe :: forall m a. (MonadIO m, Storable a) =>
    Array a -> a -> m (Array a)
snocUnsafe arr@Array{..} = snocNewEnd (INDEX_NEXT(aEnd)) arr
...
snocWith allocSize arr x = liftIO $ do
    let newEnd = INDEX_NEXT(aEnd arr)
...
getIndexPtr :: forall m a. (MonadIO m, Storable a) =>
    ArrayContents a -> Int -> Int -> Int -> m a
getIndexPtr arrContents start end i = do
    let index = INDEX_OF(start,i)
    if i >= 0 && INDEX_VALID(index,end)
...
getIndexPtrRev :: forall m a. (MonadIO m, Storable a) =>
    ArrayContents a -> Int -> Int -> Int -> m a
getIndexPtrRev arrContents start end i = do
    let index = RINDEX_OF(end,i)
    if i >= 0 && index >= start
    then liftIO $ peekWith arrContents index
...
getIndicesD liftio (D.Stream stepi sti) = Unfold step inject
    ...
    step (GetIndicesState contents start end st) = do
        ...
        case r of
            D.Yield i s -> do
                x <- liftio $ getIndexPtr contents start end i
...
reverse Array{..} = liftIO $ do
    ...
    swap l h = do
        when (l < h) $ do
            swapPtrs arrContents l h
            swap (INDEX_NEXT(l)) (INDEX_PREV(h))
...
partitionBy f arr@Array{..} = liftIO $ do
    ...
    then return (arr, arr)
        ...
        ptr <- go arrStart (INDEX_PREV(aEnd))
            ...
            then return low
                ...
                case r of
                    ...
                    Just (high1, h) -> do -- low < high1
                        ...
                        let low1 = INDEX_NEXT(low)
                            high2 = INDEX_PREV(high1)
                        if low1 <= high2
                        then go low1 high2
...
-- @since 0.7.0
{-# INLINE byteLength #-}
byteLength :: forall a. Storable a => Array a -> Int
byteLength Array{..} =
    let len = (aEnd - arrStart) * SIZE_OF(a)
    in assert (len >= 0) len
...
-- /Pre-release/
{-# INLINE byteCapacity #-}
byteCapacity :: forall a. Storable a => Array a -> Int
byteCapacity Array{..} =
    let len = (aBound - arrStart) * SIZE_OF(a)
    in assert (len >= 0) len
...
-- /Pre-release/
{-# INLINE bytesFree #-}
bytesFree :: forall a. Storable a => Array a -> Int
bytesFree Array{..} =
    let n = (aBound - aEnd) * SIZE_OF(a)
    in assert (n >= 0) n
...
arraysOf n (D.Stream step state) =
    ...
    step' gst (GroupBuffer st contents start end bound) = do
        ...
        case r of
            D.Yield x s -> do
                liftIO $ pokeWith contents end x >> touch contents
                let end1 = INDEX_NEXT(end)
                return $
                    if end1 >= bound
                    then D.Skip
                            (GroupYield
...
flattenArrays (D.Stream step state) = D.Stream step' (OuterLoop state)
    ...
    step' _ (InnerLoop st contents p end) = do
        x <- liftIO $ do
                    ...
                    touch contents
                    return r
        return $ D.Yield x (InnerLoop st contents (INDEX_NEXT(p)) end)
...
data ReadUState a = ReadUState
    UNPACKIF !(ArrayContents a)  -- contents
    !Int           -- end address
    !Int           -- current address
...
producerWith liftio = Producer step (return . toReadUState) extract
    ...
    step (ReadUState contents end cur) = do
            ...
            return $ D.Yield x (ReadUState contents end (INDEX_NEXT(cur)))
...
readRevWith liftio = Unfold step inject
    ...
    step (ReadUState contents start p) = do
        ...
        return $ D.Yield x (ReadUState contents start (INDEX_PREV(p)))
...
toStreamKWith liftio Array{..} = go arrStart
    ...
    go p | assert (p <= aEnd) (p == aEnd) = K.nil
        ...
        let elemM = do
              ...
              touch arrContents
              return r
        in liftio elemM `K.consM` go (INDEX_NEXT(p))
...
toStreamKRevWith liftio Array {..} =
    ...
    go p | p < arrStart = K.nil
        ...
        let elemM = do
              ...
              touch arrContents
              return r
        in liftio elemM `K.consM` go (INDEX_PREV(p))
...
data ArrayUnsafe a = ArrayUnsafe
    UNPACKIF !(ArrayContents a)  -- contents
    {-# UNPACK #-} !Int  -- start address
    {-# UNPACK #-} !Int  -- first unused address
...
appendNUnsafe action n =
    ...
    step (ArrayUnsafe contents start end) x = do
        liftIO $ pokeWith contents end x >> touch contents
        return $ ArrayUnsafe contents start (INDEX_NEXT(end))
...
writeNWithUnsafe alloc n = Fold step initial (return . fromArrayUnsafe)
    ...
    step (ArrayUnsafe contents start end) x = do
        ...
        return
          $ FL.Partial
          $ ArrayUnsafe contents start (INDEX_NEXT(end))
...
writeRevNWithUnsafe alloc n = Fold step initial (return . fromArrayUnsafe)
    ...
    step (ArrayUnsafe contents start end) x = do
        let ptr = INDEX_PREV(start)
        liftIO $ pokeWith contents ptr x >> touch contents
        return
          $ FL.Partial
...
writeWith elemCount =
    ...
    insertElem (Array contents start end bound) x = do
        ...
        return $ Array contents start (INDEX_NEXT(end)) bound
...
arrayStreamKLength :: Monad m => K.Stream m (Array a) -> m Int
...
spliceCopy arr1 arr2 = liftIO $ do
    let start1 = arrStart arr1
        ...
        !(I# start1InBytes#) = start1 * SIZE_OF(a)
        !(I# start2InBytes#) = start2 * SIZE_OF(a)
        !(I# len1InBytes#) = len1 * SIZE_OF(a)
        !(I# len2InBytes#) = len2 * SIZE_OF(a)
...
cmp arr1 arr2 =
    liftIO
        $ do
            let marr1 = getInternalMutableByteArray (arrContents arr1)
                marr2 = getInternalMutableByteArray (arrContents arr2)
                !(I# st1#) = arrStart arr1 * SIZE_OF(a)
                !(I# st2#) = arrStart arr2 * SIZE_OF(a)
